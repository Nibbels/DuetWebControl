        ...
        <v-card-text class="px-0 pt-0 pb-2 content text-xs-center"
                ...
                <v-row no-gutters class="flex-nowrap">
                    ...
                    <v-col>
                        <v-row align-content="center" no-gutters>
                            <v-col v-for="(axis, index) in move.axes.filter(axis => axis.visible)" :key="index"
                                   class="d-flex flex-column align-center">
                                <strong>
                                    {{ axis.letter }}
                                </strong>
                                <span>
									{{ displayToolsPosition(axis, index) }}
								</span>
                            </v-col>
                        </v-row>
                    </v-col>
                    ...
                    <v-col>
                        <v-row align-content="center" no-gutters>
                            <v-col v-for="(axis, index) in move.axes.filter(axis => axis.visible)" :key="index"
                                ...
                                <span>
									{{ displayMachinePosition(axis, index) }}
								</span>
                            </v-col>
                        </v-row>
                    </v-col>
            ...
            <template v-if="move.extruders.length">
                ...
                <v-row align-content="center" no-gutters class="flex-nowrap">
                    ...
                    <v-col>
                        <v-row align-content="center" no-gutters>
                            <v-col v-for="(extruder, index) in move.extruders" :key="index"
                                   class="d-flex flex-column align-center">
                                <strong>
            ...
            <template v-show="isNumber(move.currentMove.requestedSpeed) || isNumber(move.currentMove.topSpeed)">
                ...
                <v-row align-content="center" no-gutters class="flex-nowrap">
                    ...
                    <v-col>
                        <v-row align-content="center" no-gutters>
                            <v-col v-if="isNumber(move.currentMove.requestedSpeed)"
                                   class="d-flex flex-column align-center">
                                <strong>
            ...
            <template v-if="sensorsPresent">
                <v-divider
                        v-show="move.axes.length || move.extruders.length || isNumber(move.currentMove.requestedSpeed) || isNumber(move.currentMove.topSpeed)"
                ...
                <v-row align-content="center" no-gutters class="flex-nowrap">
                    ...
                    <v-col>
                        <v-row align-content="center" justify="center" no-gutters>
                            <v-col v-if="boards.length && boards[0].vIn.current > 0"
                                   class="d-flex flex-column align-center">
                                <strong>
                                    {{ $t('panel.status.vIn') }}
                                </strong>
                                <v-tooltip bottom>
                                    ...
                                    {{ $t('panel.status.minMax', [$display(boards[0].vIn.min, 1, 'V'),
                                    $display(boards[0].vIn.max, 1, 'V')]) }}
                                </v-tooltip>
                            </v-col>

                            <v-col v-if="boards.length && boards[0].v12.current > 0"
                                   class="d-flex flex-column align-center">
                                <strong>
                                    {{ $t('panel.status.v12') }}
                                </strong>
                                <v-tooltip bottom>
                                    ...
                                    {{ $t('panel.status.minMax', [$display(boards[0].v12.min, 1, 'V'),
                                    $display(boards[0].v12.max, 1, 'V')]) }}
                                </v-tooltip>
                            </v-col>

                            <v-col v-if="boards.length && boards[0].mcuTemp.current > -273"
                                   class="d-flex flex-column align-center">
                                ...
                                <v-tooltip bottom>
                                    ...
                                    {{ $t('panel.status.minMax', [$display(boards[0].mcuTemp.min, 1, 'C'),
                            ...
                            <v-col v-if="probesPresent" class="d-flex flex-column align-center">
                                ...
                                <div class="d-flex-inline">
									<span v-for="(probe, index) in sensors.probes" :key="index" class="pa-1 probe-span"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            displayToolsPosition(axis) {
                const position = axis.userPosition;
                return (axis.letter === 'Z') ? this.$displayZ(position, false) : this.$display(position, 1);
            },
            displayMachinePosition(axis) {
